...
class Output:
    ...
    def header(self, gui_output=None):
        ...
        print(line)
        if gui_output is not None:
            gui_output.insert(tk.END, line + "\n")
            gui_output.insert(tk.END, s + "\n")
            gui_output.insert(tk.END, line + "\n")
            gui_output.update_idletasks()
...
class Display:
    ...
    def __init__(self, output=None, attributes=None, resolution=100):
        super().__init__()
        self.output = output
        ...
        self.gui_output = None  # for GUI output, if needed

    def do(self, problem, evaluator, algorithm, pf=None):
        if not algorithm.n_gen % self.resolution == 0:
            return
        try:
            ...
            if self.display_header:
                self.output.header(self.gui_output)

            # print the actually line
            self.output.do(self.gui_output)
...
class SingleObjectiveDisplay(Display):
    ...
    def _do(self, problem, evaluator, algorithm):
        ...
        if len(feasible) > 0:
            _F = F[feasible]
            self.output.append("f_best", opt.F[0])
            if self.favg:
                self.output.append("f_avg", np.mean(_F))
        else:
            self.output.append("f_best", "-")
            if self.favg:
                self.output.append("f_avg", "-")
