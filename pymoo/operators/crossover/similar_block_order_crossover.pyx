...
class SimilarBlockOrderCrossover(Crossover):
    ...
    def _do(self, problem, x, **kwargs):
        ...
        for j in range(n_matings):
            ...
            for i in range(cut_point, n_var-1):
                # TODO: block_size
                if p1[i] == p2[i] and p1[i+1] == p2[i+1]:
                    c1[i], c1[i+1] = p1[i], p1[i+1]
                    c2[i], c2[i+1] = p1[i], p1[i+1]
