...
class MyHGA(GeneticAlgorithm):
    ...
    def __init__(self,
        ...
        self.local_search_frequency = 100
    ...
    def adapt_cross(self, pop, parents):
        ...
        if any(do_adapt):
            ...
            mutate_prob[do_adapt] = (self.mutate_prob -
                                    (self.mutate_prob-0.5)/(1+np.exp(self.pop_size*(averageFitness-parentsFitness[do_adapt])/(averageFitness-self.opt.get("F")[0][0]))))
    ...
    def _next(self):
        if self.n_gen % self.local_search_frequency == 0 or self.n_gen == 1:
            if self.gurobi_decomposed is False:
                ...
                if self.flag != 0:
                    size_groups = 500 + self.flag*100
                    self.gurobi_time_limit += 10
                ...
                for i in range(num_groups):
                    ...
                    new_best, new_best_fitness = run_gurobi_for_decomposed_problem_multi_solutions(self.problem.instance, dim_index,
                                                                                     best, self.gurobi_time_limit,
                                                                                     self.output_dir, self.n_gen)
                    best = new_best[0]
                    current_best_fitness = new_best_fitness[0]

                    # start_fitness_tuple = [np.array([-new_best_fitness[i]]) for i in range(3)]
                    # cv_value = [np.array([0]) for i in range(3)]
                    # feasible_value = [np.array([True]) for i in range(3)]

                    # new_x.extend(new_best[:3])
                    # new_y.extend(start_fitness_tuple)
                    # new_cv.extend(cv_value)
                    # new_feasible.extend(feasible_value)

                    self.n_gen += 1
                    exec_time = time.time() - self.start_time
                    self.time_history.append([self.n_gen, self.evaluator.n_eval, np.array([[-current_best_fitness]]), exec_time])
