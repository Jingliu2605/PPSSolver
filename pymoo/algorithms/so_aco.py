...
class ACO(Algorithm):
    ...
    def _next(self):
        ...
        opt = FitnessSurvival().do(self.problem, Population.merge(colony, self.opt), 1)
        ...
        if self.global_update == "all":
            ...
            ants_to_update = FitnessSurvival().do(self.problem, colony, 1)
        ...
        for ant in ants_to_update:
            for e in ant.path:
                if e.pheromone is None:
                    ...
                    self.pheromones.update(e.key, e.pheromone * self.pheromones.rho)
