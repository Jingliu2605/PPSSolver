...
from pymoo.operators.rank_and_crowding_survival import RankAndCrowdingSurvival
...
def norm_bounds(pop, problem):
    nxl = normalize(pop.get("xl"), problem.xl[0], problem.xu[0])
    nxu = normalize(pop.get("xu"), problem.xl[0], problem.xu[0])
    return nxl, nxu
...
class DIRECT(LocalSearch):
    ...
    def setup(self, problem, **kwargs):
        ...
        if self.initial_point is None:
            ...
            X = denormalize(self.initial_point, xl, xu)
        x0 = Individual(X=X)
        x0.set("xl", xl)
        x0.set("xu", xu)
        x0.set("depth", 0)
    ...
    def _next(self):
        ...
        for current in potential_optimal:
            ...
            update_bounds(current, xl, xu, k, delta)
            update_bounds(left, xl, xu, k, delta)
            update_bounds(right, xl, xu, k, delta)
