...
class GurobiSolver:
    ...
    def __init__(self, instance, time_limit=None, error_threshold=None, num_solutions=None, pool_search_mode=None, num_start=None, start_solutions=None):
    ...
    def _setup(self, time_periods, durations, values, costs, budget, capability_stream_budgets, initiation_budget,
        ...
        for t in range(bp):
            model.addConstr(
                gp.quicksum(
                    costs[j, t - t2] * x[j, t2] for j in range(self.num_projects)
                    for t2 in range(self.planning_window)
                    if 1 <= (t - t2) < durations[j]) <= ongoing_budget[t], f"Ongoing Budget({t})")
    ...
    def solve(self, verbose=False, log_to_console=False, log_file=None):
        ...
        model, variables = self._setup(time_periods, durations, values, costs, self.budget,
                                       self.capability_stream_budgets, self.initiation_budget, self.initiation_range,
        ...
        # start_memory = process.memory_info().rss / (1024 ** 3)  # Convert to MB

        # model.reset()
        model.optimize()
        # End memory tracking
        # end_memory = process.memory_info().rss / (1024 ** 3)  # Convert to GB
        # peak_memory = process.memory_info().peak_wset / (1024 ** 3)  # Windows-specific peak memory usage
        # peak_memory = process.memory_full_info().rss / (1024 ** 3) #measure maximum resident set size (maxrss) if your system supports it.
        ...
        if verbose:
            ...
            # print(f"Start memory: {start_memory:.2f} GB")
            # print(f"End memory: {end_memory:.2f} GB")
            # print(f"Peak_memory: {peak_memory:.2f} GB")
