...
class MyCCDE(GeneticAlgorithm):
    ...
    def _each_iteration(self, *args, **kwargs):
        ...
        if self.save_opt_intervals:
            ...
            self.time_history.append([exec_time, self.opt.get("F")])
    ...
    def _next(self):
        ...
        for i in range(num_groups):
            ...
            if self.var_selection == "rand":
                P = selection.do(sub_pop, self.pop_size, crossover.n_parents)

            elif self.var_selection == "best":
                best = np.argmin(F[:, 0])
                P = selection.do(sub_pop, self.pop_size, crossover.n_parents - 1)
                P = np.column_stack([np.full(len(sub_pop), best), P])

            elif self.var_selection == "rand+best":
                best = np.argmin(F[:, 0])
                P = selection.do(sub_pop, self.pop_size, crossover.n_parents)
                use_best = np.random.random(len(sub_pop)) < 0.3
                P[use_best, 0] = best
            ...
            self.off = crossover.do(self.problem, sub_pop, P, algorithm=self)

            # then do the mutation
            _pop = self.off.new().merge(sub_pop).merge(self.off)
            _P = np.column_stack([np.arange(len(sub_pop)), np.arange(len(sub_pop)) + len(sub_pop)])
            self.off = mutation.do(self.problem, _pop, _P, algorithm=self)[:len(sub_pop)]

            sub_x = self.off.get("X")
            # new = np.tile(X[best,:], (self.pop_size, 1))
            new = X
            new[:, dim_index] = sub_x
            # TODO: X[i] = X[best]
            self.off.set("X", new)
