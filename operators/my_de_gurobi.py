...
import time
import os
from operators.gurobi_operators import local_search_gurobi
...
class MyDESGurobi(GeneticAlgorithm):
    def __init__(self,
                 pop_size=100,
    ...
    def next(self):
        ...
        self.has_terminated = self.flag  # self.mip_gap < 0.002
    ...
    def _next(self):
        if not self.termination.do_continue(self):
            # run gurobi
            start_solutions, start_fitness, model = local_search_gurobi(self.pop[0], self.problem.instance, self.flag,
                                                                        self.output_dir, self.t_limitation)
