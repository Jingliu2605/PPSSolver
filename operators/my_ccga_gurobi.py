...
class MyCCGAGurobi(GeneticAlgorithm):
    ...
    def _next(self):
        ...
        for i in range(num_groups):
            ...
            self.off = self.mating.do(self.problem, sub_pop, n_offsprings=self.n_offsprings, algorithm=self)
            self.off.set("n_gen", self.n_gen)
            sub_x = self.off.get("X")
            # TODO: X[i] = X[best]
            new = np.tile(X[best,:], (self.n_offsprings, 1))
            # new = X
            new[:, dim_index] = sub_x
            self.off.set("X", new)

            # evaluate the offspring
            self.evaluator.eval(self.problem, self.off, algorithm=self, dim_index=dim_index, best_now=X[best,:])
