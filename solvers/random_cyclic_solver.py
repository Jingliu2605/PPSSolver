...
class RandomCyclicSolver(HeuristicSolver):
    """
    Generate a random, feasible solution using random cyclic project selection.

    While not completed:
        For each time step in a random order:
            Select feasible project (if available)
            Continue to next time step (in random order)
        If no projects selected, mark completed
    """

    def __init__(self, instance, seed=1):
        super().__init__(instance, seed)
        self.name = "Random Cyclic"
    ...
    def solve(self):
        ...
        while not done:
            ...
            order = np.random.permutation(range(1, self.instance.planning_window + 1))
